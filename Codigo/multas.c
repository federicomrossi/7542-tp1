...
// PRE: 'consumo_tipico', entero que representa el consumo tipico para ese cliente; 
// 'factor_de_multa', entero que expresa el factor de multa a aplicar; 
// 'minutos_multables', entero que expresa la cantidad de minutos a multar.
...
// Función que envía a la salida estandar el número de cliente y el monto de la multa
// a aplicarle, con el formato CLIENTE:MULTA.
// PRE: 'num_cliente', número de cliente; 'multa', monto entero de la multa del cliente.
...
// Función que dado los parámetros a especificar, genera la multa correspondiente
// y la envia a la salida estandar.
// PRE: 'x_p', factor de multa a aplicar; 'cliente', número del cliente a quien se va a
// aplicar la multa; 'consumo_tipico', consumo típico registrado por el cliente; 
// 'minutos_acumulados', minutos acumulados de interrupciones; 'flag_tolerancia_superada', 
// sensor booleano que expresa si se ha superado alguna tolerancia.
void generar_multa(int x_p, int cliente, int consumo_tipico, int minutos_acumulados,
...
// de un cliente.
// PRE: 'x_m', cantidad máxima de minutos para la cual se considera momentanea una
// interrupción; 'x_f', cantidad máxima de interrupciones tolerables; 'x_d', cantidad
// máxima de minutos acumulados tolerables; 'duracion_interrupcion', duración en 
// minutos de la interrupción a procesar del cliente; cant_interrupciones, entero 
// que especifica la cantidad de interrupciones registradas hasta el momento para el 
// cliente; 'minutos_acumulados', puntero a variable que almacena los minutos acumulados
// de interrupciones; 'flag_tolerancia_superada', puntero a variable booleana que 
// expresa si se ha superado alguna tolerancia.
// POST: el flag de tolerancia superada se seteara en true si alguna tolerancia o todas
// a la vez son superadas; 'minutos_acumulados' adoptará un nuevo valor de minutos de 
// acuerdo a la tolerancia superada. En caso de superarse todas a la vez, se considera 
// aquella que registre mas cantidad de minutos acumulados.
void evaluar_tolerancias(int x_m, int x_f, int x_d, int duracion_interrupcion, 
	int cant_interrupciones, int *minutos_acumulados, bool *flag_tolerancia_superada)
{
...
// Función que se encarga de aplicar filtros y tolerancias a las interrupciones de los
// clientes.
// PRE: 'x_m', cantidad máxima de minutos para la cual se considera momentanea una
// interrupción; 'x_f', cantidad máxima de interrupciones tolerables; 'x_d', cantidad
// máxima de minutos acumulados tolerables; 'duracion_interrupcion', duración en minutos 
// de la interrupción a procesar del cliente; 'cant_interrupciones', puntero a variable 
// entera que posea la cantidad de interrupciones registradas hasta el momento para el 
// cliente; 'minutos_acumulados', puntero a variable que almacena los minutos acumulados
// de interrupciones; 'flag_tolerancia_superada', puntero a variable booleana que expresa 
// si se ha superado alguna tolerancia.
// POST: 'cant_interrupciones' es incrementado si no son aplicables los filtros; 
// 'minutos_acumulados' es actualizado de acuerdo a los filtros y tolerancias.
		...
		*minutos_acumulados += duracion_interrupcion;
		...
		if (!(*flag_tolerancia_superada))
		{
			evaluar_tolerancias(x_m, x_f, x_d, duracion_interrupcion, 
				*cant_interrupciones, minutos_acumulados, flag_tolerancia_superada);
		}
	}
}
...
// Función que procesa las interrupciones del servicio ocurridas y registradas en 
// un archivo, aplicando las multas pertinentes a cada cliente.
// PRE: 'x_m', cantidad máxima de minutos para la cual se considera momentanea una
// interrupción; 'x_f', cantidad máxima de interrupciones tolerables; 'x_d', cantidad
// máxima de minutos acumulados tolerables; 'x_p', entero que establece el factor 
// de multa a aplicar; 'archivo', nombre de archivo (incluyendo su extensión) donde 
// se registran las interrupciones y el cliente al que pertenece cada una de ellas,
// dispuestos con el formato [numero_cliente]:[consumo_tipico]:[duracion_interrupcion].
